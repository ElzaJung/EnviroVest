  ...
  const fetchData = async () => {
    try {
      ...
      const { data: sentimentData, error: sentimentError } = await supabase
        .from("sentiment_data")
        .select(
          "article_title, article_text, article_published, article_top_image, article_resolved_url"
        )
        .eq("ticker", ticker);
  ...
  return (
    ...
    >
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "2rem" }}>
        ...
        <p style={{ color: "#ccc" }}>{social_summary || "Not available"}</p>
        <h3 style={{ marginTop: "1rem" }}>Governance (G) Summary:</h3>
        <p style={{ color: "#ccc" }}>{governance_summary || "Not available"}</p>
        ...
        {sectorCompanies.length > 0 && (
          <div style={{ marginTop: "2rem" }}>
            ...
            {isLineChart ? (
              ...
              <Bar data={getSectorChartData()} options={{ responsive: true }} />
            )}
          </div>
        )}
          ...
          {sentiment_data.map((article, index) => (
            <NewsCard
              ...
              articleTopImage={
                ...
                article.article_top_image.length > 0
                  ? article.article_top_image
                  : "https://img.freepik.com/free-vector/flat-background-dia-del-periodista-celebration_23-2149525339.jpg"
              }
